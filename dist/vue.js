...
(function (global, factory) {
  ...
  function set(target, key, val) {
    ...
    // $flow-disable-line
    var ob = target.__ob__; // 获取targer对应的observer实例
  ...
  function normalizeChildren(children) {
    return isPrimitive(children) // 原始值
  ...
  function resolveInject(inject, vm) {
    if (inject) {
      ...
      */
       ...
       */
      for (var i = 0; i < keys.length; i++) {
        var key = keys[i]; // key对应的值 {from, default}

        // 存在 __ob__属性说明已经做过响应式处理了
        if (key === "__ob__") { continue; }

        var provideKey = inject[key].from; // 属性名
        var source = vm;
  ...
  var componentVNodeHooks = {
    ...
    insert: function insert(vnode) {
      ...
      if (!componentInstance._isMounted) {
        ...
        // console.log("insert ", componentInstance._uid);
  ...
  function lifecycleMixin(Vue) {
    ...
    Vue.prototype._update = function (vnode, hydrating) {
      var vm = this;
      var prevEl = vm.$el;
      var prevVnode = vm._vnode; // old VNode
  ...
  function queueWatcher(watcher) {
    ...
    if (has[id] == null) {
      ...
      if (!flushing) {
        ...
        // 处理执行队列中的watcher 过程中产生的新的watcher
        /**
  ...
  function initInternalComponent(
    ...
    var parentVnode = options._parentVnode; // 组件的 VNode
    opts.parent = options.parent; // 父组件实例
    opts._parentVnode = parentVnode;

    var vnodeComponentOptions = parentVnode.componentOptions; // 组件选项
  ...
  function createPatchFunction(backend) {
    ...
    function initComponent(vnode, insertedVnodeQueue) {
      if (isDef(vnode.data.pendingInsert)) {
        ...
        // 根实例创建真实dom时，会传入insertedVnodeQueue，收集子组件的实例
    ...
    function patchVnode(
      ...
      if (isUndef(vnode.text)) {
        ...
        if (isDef(oldCh) && isDef(ch)) {
          ...
          // 如果旧的节点是文本节点则先将节点的文本清除，然后通过 addVnodes 将 ch 批量插入到新节点 elm 下
    ...
    function invokeInsertHook(vnode, queue, initial) {
       ...
       * 因为子组件生成真实Dom后，都会走到这里，当判断为组件为初次渲染且有父vnode
         就不进行遍历queue，而是把队列保留在data.pendingInsert属性里，供后续父实例拿到当前队列
    ...
    return function patch(oldVnode, vnode, hydrating, removeOnly) {
      // console.log(vnode.context._uid, "patch");
      ...
      if (isUndef(oldVnode)) {
        ...
        var isRealElement = isDef(oldVnode.nodeType); // 是否是元素
        ...
        if (!isRealElement && sameVnode(oldVnode, vnode)) {
          ...
          // 删除旧节点
          if (isDef(parentElm)) {
            // 如果父节点存在，则执行 removeVnodes：把 oldVnode 从当前 DOM 树中删除
  ...
  Vue.prototype.$mount = function (
    ...
    if (!options.render) {
      // 有template 取template，如果没有则取el下的html作为template
